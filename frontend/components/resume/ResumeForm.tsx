        ...
        <>
          ...
          <div>
            ...
            <Input
              ...
              placeholder="e.g. Role, Issuer, Technology"
            />
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <Label
              htmlFor={`custom-current-${item._id}`}
              className="cursor-pointer"
            >
              Ongoing / Present
            </Label>
...
// ─────────────────────────────────────────────
// Custom Section Block (one whole section)
// ─────────────────────────────────────────────
...
export default function ResumeForm({
  ...
  const handlePersonalInfoChange = (field: string, value: string) => {
    onChange({ personalInfo: { ...resume.personalInfo, [field]: value } });
  };
  ...
  const handleAddSkill = (skill: string) => {
    if (!skill.trim()) return;
    onChange({ skills: [...(resume.skills || []), skill.trim()] });
  };
  ...
  const handleUpdateCustomSection = (index: number, updates: any) => {
    ...
    onChange({ customSections: newSections });
  };
  ...
  const handleDragEnd = (event: any) => {
    ...
    if (active.id !== over.id) {
      const oldIndex = resume.experience.findIndex(
        (exp: any) => exp._id === active.id || exp.company === active.id
      );
      const newIndex = resume.experience.findIndex(
        (exp: any) => exp._id === over.id || exp.company === over.id
      );
      onChange({
        experience: arrayMove(resume.experience, oldIndex, newIndex),
      });
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <ColorPicker
              color={resume.theme?.primaryColor || "#3b82f6"}
              onChange={(color) =>
                onChange({ theme: { ...resume.theme, primaryColor: color } })
              }
            />
          </div>
          <div>
            ...
            <Select
              value={resume.theme?.font || "inter"}
              onValueChange={(value) =>
                onChange({ theme: { ...resume.theme, font: value } })
              }
            >
              <SelectTrigger>
                <SelectValue />
            ...
            <SortableContext
              items={
                resume.experience?.map(
                  (exp: any, i: number) => exp._id || i.toString()
                ) || []
              }
              strategy={verticalListSortingStrategy}
            >
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Add Projects, Certifications, Awards, Languages, or any custom
            section.
          </p>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          {(resume.customSections || []).map((section: any, index: number) => (
            <CustomSectionBlock
              key={index}
              section={section}
              onUpdate={(updates) => handleUpdateCustomSection(index, updates)}
